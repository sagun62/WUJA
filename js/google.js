	...
	service.getDetails({ placeId }, (place, status) => {
		if (status === google.maps.places.PlacesServiceStatus.OK) {
			...
			reviewsSummary.innerHTML = `
				<p>Average rating from <img src="img/google.png" class="review__img"/>  <strong>${averageRating.toFixed(
					1
				)} of 5</strong>, based on <strong>${totalReviews} reviews</strong></p>`;
			...
			place.reviews.forEach((review) => {
				...
				reviewElement.innerHTML = `
					...
					<a href="https://www.google.com/maps/place/?q=place_id:${placeId}" onclick="void(0)"class="read-more" target="_blank">Full Review &nbsp;<i class="fa-solid fa-arrow-right ml"></i></a>
				`;
...
function updateSlider() {
	const slideWidth = slider.children[0].offsetWidth; // Pobierz szerokość slajdu
